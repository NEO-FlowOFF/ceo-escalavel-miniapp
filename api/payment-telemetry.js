...
export default async function handler(req, res) {
  ...
  if (req.method === 'POST') {
    const {
      eventType, itemId, itemTitle, itemPrice,
      userId, username, source, appVersion,
      invoiceStatus, errorMessage, failureStage, timestamp
    } = req.body || {};
    ...
    try {
      await redis.set(eventId, JSON.stringify(eventPayload), 'EX', 60 * 60 * 24 * 90);
      await redis.zadd(TIMELINE_KEY, now, eventId);
      await Promise.all([
        redis.incr(`payments:count:${eventType}`),
        redis.incr(`payments:count:${eventType}:${eventPayload.day}`)
      ]);
  ...
  if (req.method === 'GET') {
    ...
    try {
      const [created, paid, failed, eventIds] = await Promise.all([
        redis.get('payments:count:invoice_created'),
        redis.get('payments:count:invoice_paid'),
        redis.get('payments:count:invoice_failed'),
        redis.zrevrange(TIMELINE_KEY, 0, limit - 1)
      ]);
